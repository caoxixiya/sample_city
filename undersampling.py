...
writer = SummaryWriter('logs_5000')
...
# train_dataset = data.dataset[train_dataset.indices]
# test_dataset = data.dataset[test_dataset.indices]
train_label = train_dataset[:, -1]

index_unstable = np.where(train_label==1)[0]
index_stable = np.where(train_label==0)[0][:index_unstable.shape[0]]
train_unstable_size = index_unstable.shape[0]
